...
class MockLoadStep:
    ...
    def __init__(
        self, source_name="test_source", table_name="test_table", mode="REPLACE"
    ):
...
def v2_available_executor():
    ...
    executor._source_definitions = {"test_source": {"type": "csv", "path": "test.csv"}}
    return executor
...
class TestV2CompatibilityBridge:
    ...
    def test_v1_to_v2_load_step_conversion(self, v2_available_executor):
        ...
        v1_load_step = MockLoadStep(
            source_name="customers.csv", table_name="customers", mode="UPSERT"
        )
