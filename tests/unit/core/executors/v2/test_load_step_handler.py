...
class TestLoadStepHandler:
    ...
    def test_connector_creation_error_handling(
        self, mock_execution_context, sample_load_step
    ):
        """Test error handling when connector creation fails."""
        handler = LoadStepHandler()

        # Make connector registry raise an error
        mock_execution_context.connector_registry.create_source_connector.side_effect = ValueError(
    ...
    def test_successful_execution_structure(self, mock_execution_context):
        ...
        try:
            ...
            mock_execution_context.connector_registry.create_source_connector.return_value = (
                mock_connector
            )
    ...
    def test_observability_integration(self, mock_execution_context):
        ...
        mock_execution_context.connector_registry.create_source_connector.side_effect = RuntimeError(
            "Test error"
        )
    ...
    def test_result_structure_with_error(
        ...
        # Force an error
        mock_execution_context.connector_registry.create_source_connector.side_effect = Exception(
            "Test error"
        )
