...
**Note:** The following tasks (2.6 - 2.15) are for MVP refinement and stabilization of the UDF feature, addressing issues identified post-initial completion, including the skipped complex integration test and panel feedback.

| Task | Description | Status | Assignee |
|------|-------------|--------|----------|
| [Task 2.6](#task-26-standardize-table-udf-signature--decorator-logic) | Standardize Table UDF Signature & Decorator Logic | ⬜ NOT STARTED | |
| [Task 2.7](#task-27-refactor-duckdb-engine-for-table-udf-registration) | Refactor DuckDB Engine for Table UDF Registration | ⬜ NOT STARTED | |
| [Task 2.8](#task-28-resolve-complex-udf-integration-test) | Resolve Complex UDF Integration Test | ⬜ NOT STARTED | |
| [Task 2.9](#task-29-enhance-udf-unit-testing-table-udfs--engine) | Enhance UDF Unit Testing (Table UDFs & Engine) | ⬜ NOT STARTED | |
| [Task 2.10](#task-210-improve-udf-discovery--metadata) | Improve UDF Discovery & Metadata | ⬜ NOT STARTED | |
| [Task 2.11](#task-211-enhance-udf-lifecycle-error-reporting) | Enhance UDF Lifecycle Error Reporting | ⬜ NOT STARTED | |
| [Task 2.12](#task-212-verify-and-polish-cli-for-udfs) | Verify and Polish CLI for UDFs | ⬜ NOT STARTED | |
| [Task 2.13](#task-213-update-udf-documentation-for-mvp) | Update UDF Documentation for MVP | ⬜ NOT STARTED | |
| [Task 2.14](#task-214-review-and-refine-sqlengine-udf-interface) | Review and Refine SQLEngine UDF Interface | ⬜ NOT STARTED | |
| [Task 2.15](#task-215-create-basic-end-to-end-udf-demo-for-mvp) | Create Basic End-to-End UDF Demo for MVP | ⬜ NOT STARTED | |
...
3. Correctly map the UDF signature to DuckDB\'s `create_table_function` (or `table_function` for older versions) requirements. This may involve handling named arguments.
4. If DuckDB fails to register the UDF (e.g., due to signature mismatch, unsupported types by DuckDB for the given parameters), raise an informative `RegistrationError` with details about the UDF name and the likely cause.
...
- Unit tests for `DuckDBEngine.register_python_udf` specifically for table UDFs:
    ...
    - Informative error messages for registration failures (e.g., if UDF signature is incompatible with DuckDB).
- Integration tests verifying that table UDFs, once registered, are correctly callable and executable within DuckDB queries.
