...
class TestV1V2CompatibilityBridge:
    ...
    def test_replace_mode_produces_identical_results(self, test_csv_data):
        ...
        # Get row counts
        v1_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_v1").fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_v2").fetchone()[0]
        
        # Row counts must be identical
        assert v1_count == v2_count
        logger.info(f"REPLACE mode compatibility verified: both V1 and V2 loaded {v1_count} rows")
    ...
    def test_append_mode_produces_identical_results(self, test_csv_data):
        ...
        initial_v1_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_append_v1").fetchone()[0]
        ...
        final_v1_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_append_v1").fetchone()[0]
        ...
        initial_v2_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_append_v2").fetchone()[0]
        ...
        final_v2_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_append_v2").fetchone()[0]
        ...
        assert final_v1_count == initial_v1_count  # Same count because LocalExecutor replaces
        assert final_v2_count == initial_v2_count  # Same count because LocalExecutor replaces
        
        logger.info(f"APPEND mode compatibility verified: V1 {initial_v1_count}->{final_v1_count}, V2 {initial_v2_count}->{final_v2_count}")
    ...
    def test_error_handling_consistency(self, test_csv_data):
        ...
        logger.info(f"Error handling consistency verified: both V1 and V2 returned status '{v1_result['status']}'")
    ...
    def test_multiple_loads_produce_identical_state(self, test_csv_data):
        ...
        v1_customers_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_multi_v1").fetchone()[0]
        v2_customers_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM customers_multi_v2").fetchone()[0]
        
        v1_orders_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM orders_multi_v1").fetchone()[0]
        v2_orders_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM orders_multi_v2").fetchone()[0]
        
        assert v1_customers_count == v2_customers_count
        assert v1_orders_count == v2_orders_count
        
        logger.info(f"Multiple loads compatibility verified: customers V1={v1_customers_count} V2={v2_customers_count}, orders V1={v1_orders_count} V2={v2_orders_count}")
    ...
    def test_concurrent_v1_v2_isolation(self, test_csv_data):
        ...
        v1_count = v1_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM isolation_v1").fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query("SELECT COUNT(*) FROM isolation_v2").fetchone()[0]
        
        assert v1_count > 0
        assert v2_count > 0
        assert v1_count == v2_count  # Should have same amount of data
        
        logger.info(f"Concurrent V1/V2 isolation verified: V1={v1_count} rows, V2={v2_count} rows") 