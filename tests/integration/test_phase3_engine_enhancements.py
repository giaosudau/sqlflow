...
def sample_table_udf():
    ...
    def sample_analytics(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "value" in df.columns:
            ...
            result["category"] = df["value"].apply(
    ...
    setattr(
        sample_analytics,
        "_output_schema",
        {"value": "INTEGER", "calculated": "INTEGER", "category": "VARCHAR"},
    )
...
class TestPhase3EngineEnhancements:
    ...
    def test_performance_metrics_insights(self, engine, sample_table_udf):
        ...
        if (
            metrics["table_udf_specific"]["vectorized_udfs"]
            < metrics["table_udf_specific"]["total_table_udfs"]
        ):
