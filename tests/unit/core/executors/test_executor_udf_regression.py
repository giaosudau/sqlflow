...
class TestExecutorUDFRegression:
    ...
    def test_udf_registration_called_during_initialization(self, mock_register, temp_udf_project_simple):
    ...
    def test_udf_query_processing_called_during_sql_execution(self, mock_process_query):
        ...
        mock_process_query.return_value = "CREATE TABLE test AS SELECT processed_name FROM test_table"
        
        executor = LocalExecutor()
        executor.discovered_udfs = {'python_udfs.simple_udfs.test_func': lambda x: f"processed_{x}"}
        ...
        result = executor._execute_sql_query("test", sql_with_udf)
...
class TestExecutorSourceDefinitionRegression:
    ...
    def test_source_definition_parameter_location(self):
        ...
        assert executor.source_definitions['test_source1']['params'] == {'path': 'data/test1.csv', 'has_header': True}
        ...
        assert executor.source_definitions['test_source2']['params'] == {'path': 'data/test2.csv', 'has_header': True}
    ...
    def test_source_definition_connector_type_field_compatibility(self):
        ...
        assert result['status'] == 'success'
        assert executor.source_definitions['test_source_legacy']['connector_type'] == 'CSV' 