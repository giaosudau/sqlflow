...
def observed_execution(step_type: str, *, 
                      ...
                      record_resources: bool = False) -> Callable[[F], F]:
    """
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(handler_self, step: BaseStep, context: ExecutionContext) -> StepExecutionResult:
            ...
            try:
                ...
                # Calculate execution time for failure recording
                ...
                try:
                    context.observability_manager.record_step_failure(
                        step_id, str(e), execution_time_ms
                    )
                except Exception as obs_error:
                ...
                logger.error(f"Failed to execute {step_type} step {step_id}: {e}")
...
def _enrich_result_with_observability(
    ...
    if record_performance and hasattr(result, 'performance_metrics'):
        ...
        try:
            ...
            if hasattr(result, 'performance_metrics') and hasattr(result.performance_metrics, 'update'):
