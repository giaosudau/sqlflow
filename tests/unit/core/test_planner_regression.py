...
class TestPlannerRegression:
    ...
    def test_source_definition_uses_source_connector_type(self):
        ...
        source_steps = [step for step in plan if step.get('type') == 'source_definition']
        ...
        assert 'connector_type' not in source_step or source_step.get('connector_type') == source_step['source_connector_type']
        ...
        for step in plan:
            ...
            for dependency_id in depends_on:
                ...
                dependency_exists = any(
                    s.get('id') == dependency_id for s in plan
                )
                assert dependency_exists, f"Dependency {dependency_id} not found in plan"
...
class TestPlannerExecutionOrderRegression:
    ...
    def test_load_before_transform_dependency(self):
        ...
        for i, step in enumerate(plan):
            if step.get('type') == 'load' and step.get('target_table') == 'raw_customers':
                load_index = i
            elif step.get('type') == 'transform' and step.get('name') == 'processed_customers':
                transform_index = i
