...
class TestTableUDFPerformance:
    ...
    def test_performance_benchmarks_small_dataset(self, duckdb_engine: DuckDBEngine):
        ...
        test_data = pd.DataFrame(
            {
                ...
                "original_value": np.random.random(
                    self.PERFORMANCE_TARGETS["small_dataset"]["rows"]
                ),
            }
        )
        ...
        assert execution_time < max_time, (
            f"Execution took {execution_time:.2f}ms, expected < {max_time}ms"
        )
    ...
    @pytest.mark.skipif(True, reason="psutil not available for memory testing")
    ...
    def test_performance_regression_prevention(self, duckdb_engine: DuckDBEngine):
        ...
        @python_table_udf(output_schema={"id": "INTEGER", "baseline_result": "DOUBLE"})
