...
INCOMPATIBLE_COMBINATIONS = [
    ...
    ({"sync_mode": "incremental"}, set(), "cursor_field"),  # incremental requires cursor_field
]
...
class SourceParameterValidator:
    ...
    def _validate_sync_mode(self, params: Dict[str, Any]) -> List[str]:
        ...
        if sync_mode is not None:
            if not isinstance(sync_mode, str):
                ...
                errors.append(f"Invalid sync_mode '{sync_mode}'. Must be one of: {valid_modes}")
    ...
    def _validate_primary_key(self, params: Dict[str, Any]) -> List[str]:
        ...
        if primary_key is not None:
            if isinstance(primary_key, str):
                ...
                elif not all(isinstance(key, str) and key.strip() for key in primary_key):
    ...
    def _validate_destination_sync_mode(self, params: Dict[str, Any]) -> List[str]:
        ...
        if dest_sync_mode is not None:
            if not isinstance(dest_sync_mode, str):
                ...
                valid_modes = ", ".join(sorted(STANDARD_PARAMETERS["destination_sync_mode"]))
                errors.append(f"Invalid destination_sync_mode '{dest_sync_mode}'. Must be one of: {valid_modes}")
    ...
    def _validate_replication_method(self, params: Dict[str, Any]) -> List[str]:
        ...
        if replication_method is not None:
            if not isinstance(replication_method, str):
                ...
                valid_methods = ", ".join(sorted(STANDARD_PARAMETERS["replication_method"]))
                errors.append(f"Invalid replication_method '{replication_method}'. Must be one of: {valid_methods}")
    ...
    def _validate_required_dependencies(self, params: Dict[str, Any]) -> List[str]:
        ...
        if sync_mode == "incremental" and cursor_field is None:
            errors.append("sync_mode 'incremental' requires cursor_field to be specified")
            
        # append_dedup destination_sync_mode requires primary_key
        if destination_sync_mode == "append_dedup" and primary_key is None:
            errors.append("destination_sync_mode 'append_dedup' requires primary_key to be specified")
    ...
    def get_migration_suggestions(self, params: Dict[str, Any]) -> List[str]:
        ...
        if "replication_method" in params and "sync_mode" not in params:
            ...
            if replication_method == "FULL_TABLE":
                suggestions.append("Consider using sync_mode: 'full_refresh' instead of replication_method: 'FULL_TABLE'")
            elif replication_method == "INCREMENTAL":
                suggestions.append("Consider using sync_mode: 'incremental' instead of replication_method: 'INCREMENTAL'")
            elif replication_method == "LOG_BASED":
                suggestions.append("Consider using sync_mode: 'cdc' instead of replication_method: 'LOG_BASED'")
                
        # Check for missing recommended parameters
        if params.get("sync_mode") == "incremental" and "lookback_window" not in params:
            suggestions.append("Consider adding lookback_window for incremental sync to handle late-arriving data")
