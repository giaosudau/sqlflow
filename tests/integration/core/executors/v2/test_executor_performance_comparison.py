...
class TestV1V2PerformanceComparison:
    ...
    def test_replace_mode_performance_comparison(self, test_dataset):
        ...
        v1_count = v1_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM perf_test_v1"
        ).fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM perf_test_v2"
        ).fetchone()[0]
        ...
        logger.info("REPLACE mode performance comparison:")
        ...
        ), f"V2 is significantly slower than V1: {performance_ratio:.3f}x"
    ...
    def test_append_mode_performance_comparison(self, test_dataset):
        ...
        v1_count = v1_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM append_test_v1"
        ).fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM append_test_v2"
        ).fetchone()[0]
        assert v1_count == v2_count
        ...
        logger.info("APPEND mode performance comparison:")
        logger.info(f"  V1 duration: {v1_duration:.3f}s")
        logger.info(f"  V2 duration: {v2_duration:.3f}s")
        logger.info(f"  Performance ratio (V2/V1): {performance_ratio:.3f}")
    ...
    def test_medium_dataset_performance_comparison(self, test_dataset):
        ...
        # Verify identical row counts
        v1_count = v1_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM medium_test_v1"
        ).fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM medium_test_v2"
        ).fetchone()[0]
        assert v1_count == v2_count
        ...
        logger.info("Medium dataset performance comparison:")
        logger.info(f"  V1 duration: {v1_duration:.3f}s")
        logger.info(f"  V2 duration: {v2_duration:.3f}s")
    ...
    def test_execution_result_structure_comparison(self, test_dataset):
        ...
        logger.info("  Compatible core structure: ✓")
    ...
    def test_concurrent_v1_v2_execution(self, test_dataset):
        ...
        v1_count = v1_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM concurrent_v1_table"
        ).fetchone()[0]
        v2_count = v2_executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM concurrent_v2_table"
        ).fetchone()[0]
        ...
        logger.info(
            f"Concurrent execution verified: V1={v1_count} rows, V2={v2_count} rows"
        )
    ...
    def test_error_handling_consistency(self, test_dataset):
        ...
        logger.info(
            f"Error handling consistency: both returned status '{v1_result['status']}'"
        )
    ...
    def test_observability_enhancement_validation(self, test_dataset):
        ...
        logger.info("  V2 execution completed with enhanced monitoring")
        logger.info("  Result structure maintained for compatibility")
    ...
    def test_migration_path_validation(self, test_dataset):
        ...
        v1_count = executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM migration_test_table"
        ).fetchone()[0]
        ...
        v2_count = executor.duckdb_engine.execute_query(
            "SELECT COUNT(*) FROM migration_test_table_v2"
        ).fetchone()[0]
        ...
        logger.info("Migration path validated: V1→V2 transition successful")
