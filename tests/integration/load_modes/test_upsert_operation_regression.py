"""Integration tests for upsert operation regression prevention.

These tests ensure that the UPSERT mode with upsert keys functionality
doesn't regress. Specifically, they test that upsert_keys are properly
passed through the parsing -> planning -> execution pipeline. Previously,
the upsert_keys parameter was missing, causing "UPSERT operation requires
at least one upsert key" errors even when upsert_keys were correctly specified.
"""
...
class TestUpsertOperationRegression:
    """Test UPSERT operations to prevent regression of executor fix."""

    @pytest.fixture
    def setup_test_data(self):
        """Create test data files for UPSERT operations."""
    ...
    def test_upsert_operation_with_single_key(self, setup_test_data):
        """Test UPSERT operation with single upsert key executes successfully."""
        data = setup_test_data

        # Create pipeline with UPSERT operation
        pipeline_sql = f"""
        ...
        LOAD users_table FROM updates_csv MODE UPSERT KEY (user_id);
        ...
        # Verify UPSERT operation worked correctly
    ...
    def test_upsert_operation_with_multiple_keys(self, setup_test_data):
        """Test UPSERT operation with multiple upsert keys."""
        data = setup_test_data
        ...
        LOAD sales_table FROM updates_csv MODE UPSERT KEY (user_id, region);
        ...
        # Should have 5 rows (2 updated, 2 unchanged, 1 completely new since user 3 region US doesn't exist in original)
        ...
        # Check that new record was inserted (user 3 region US)
        user3_us = result_df[
            (result_df["user_id"] == 3) & (result_df["region"] == "US")
        ]
        assert len(user3_us) == 1, "User 3 US should be inserted as a new record"
        assert user3_us.iloc[0]["sales"] == 180

    def test_upsert_operation_execution_plan_structure(self, setup_test_data):
        """Test that execution plan correctly preserves upsert_keys."""
        data = setup_test_data
        ...
        # Find the UPSERT operation in the plan
        upsert_step = None
        for step in execution_plan:
            if (
                step.get("type") == "load"
                and step.get("mode") == "UPSERT"
                and "upsert_keys" in step
            ):
                upsert_step = step
                break

        # Verify upsert step exists and has correct structure
        assert upsert_step is not None, "UPSERT step not found in execution plan"
        assert upsert_step["upsert_keys"] == ["user_id", "email"]
        assert upsert_step["mode"] == "UPSERT"
        assert "source_name" in upsert_step
    ...
    def test_load_step_creation_from_execution_plan(self, setup_test_data):
        ...
        load_step = LoadStep(
            ...
            upsert_keys=execution_step.get(
                "upsert_keys", []
            ),  # Updated for upsert_keys
        )
        ...
        assert load_step.upsert_keys == ["test_key"]
    ...
    def test_regression_prevention_comprehensive(self, setup_test_data):
        ...
        -- This UPSERT operation was failing with "missing upsert key" before fix
        LOAD target_table FROM update_data MODE UPSERT KEY (user_id);
        """
        ...
        upsert_steps = [
            step
            for step in execution_plan
            if step.get("type") == "load" and step.get("mode") == "UPSERT"
        ]
        assert len(upsert_steps) == 1, "Should have exactly one UPSERT step"
        assert upsert_steps[0]["upsert_keys"] == [
            "user_id"
