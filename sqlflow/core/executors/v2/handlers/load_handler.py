...
class LoadStepHandler(StepHandler):
    ...
    def execute(self, step: LoadStep, context: ExecutionContext) -> StepExecutionResult:
        ...
        logger.debug(f"Executing LoadStep {step.id}: {step.source} -> {step.target_table}")
    ...
    def _create_source_connector(self, step: LoadStep, context: ExecutionContext):
        ...
        try:
            ...
            if hasattr(context, 'profile_manager') and context.profile_manager:
                try:
                    source_config = context.profile_manager.get_source_config(step.source)
                    if source_config:
                        # Use profile-based configuration
                        resolution_result = context.connector_registry.resolve_configuration(
    ...
    def _load_data_from_source(self, connector, step: LoadStep, context: ExecutionContext):
        """
        ...
        try:
            ...
            if hasattr(connector, 'read_chunks'):
                ...
                data_chunks = list(connector.read_chunks())
                ...
                elif hasattr(data_result, '__iter__') and not isinstance(data_result, (str, bytes)):
                    # Iterator/generator of chunks
                    for data_item in data_result:
                        if isinstance(data_item, DataChunk):
                            data_chunks.append(data_item)
                        elif isinstance(data_item, pd.DataFrame):
                            data_chunks.append(DataChunk(data_item))
                        else:
                ...
                raise ValueError(f"Connector {type(connector)} does not support data reading")
            ...
            raise ValueError(f"Failed to load data from source '{step.source}': {e}") from e
    
    def _process_load_mode(self, step: LoadStep, context: ExecutionContext, 
                          data_chunks: List[DataChunk], total_rows: int) -> Dict[str, Any]:
        """
    ...
    def _process_replace_mode(self, step: LoadStep, context: ExecutionContext, 
                             data_chunks: List[DataChunk]) -> Dict[str, Any]:
    ...
    def _process_append_mode(self, step: LoadStep, context: ExecutionContext, 
                            data_chunks: List[DataChunk]) -> Dict[str, Any]:
    ...
    def _process_upsert_mode(self, step: LoadStep, context: ExecutionContext, 
                            data_chunks: List[DataChunk]) -> Dict[str, Any]:
        """Process UPSERT mode - merge data based on key columns."""
        if not step.incremental_config or not step.incremental_config.get("upsert_keys"):
        ...
        for i, chunk in enumerate(remaining_chunks):
            ...
            context.sql_engine.execute_query(f"""
    ...
    def _update_watermarks_if_needed(self, step: LoadStep, context: ExecutionContext, 
        ...
        try:
            ...
            if max_cursor_value is not None:
                ...
                logger.debug(f"Updated watermark for {step.id}: {cursor_field} = {max_cursor_value}")
                
        except Exception as e:
            logger.warning(f"Failed to update watermarks for step {step.id}: {e}")
    
    def _collect_performance_metrics(self, result_info: Dict[str, Any], 
    ...
    def _collect_schema_information(self, step: LoadStep, context: ExecutionContext, 
                                  data_chunks: List[DataChunk]) -> Dict[str, Any]:
        ...
        try:
            ...
            if data_chunks:
                first_chunk = data_chunks[0]
                schema_info["input_schemas"][step.source] = list(first_chunk.schema.names)
            ...
            logger.warning(f"Failed to collect schema information for step {step.id}: {e}")
    ...
    def _provide_user_feedback(self, step: LoadStep, total_rows: int) -> None:
        ...
        logger.debug(f"Load operation completed: {step.source} -> {step.target_table} ({total_rows} rows)") 