...
class TestUDFDiscovery:
    ...
    def test_discovery_single_file_finds_all_udfs(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        expected_names = ["python_udfs.basic_udfs.add_ten", "python_udfs.basic_udfs.format_name", 
                         "python_udfs.basic_udfs.calculate_tax", "python_udfs.basic_udfs.enrich_data", 
        ...
        scalar_udfs = [name for name in udf_names if manager.get_udf_info(name)["type"] == "scalar"]
        table_udfs = [name for name in udf_names if manager.get_udf_info(name)["type"] == "table"]
        
        assert len(scalar_udfs) >= 3
        assert len(table_udfs) >= 2

    def test_discovery_multiple_files_with_subdirectories(self, core_udf_test_env: Dict[str, Any]) -> None:
    ...
    def test_discovery_ignores_non_python_files(self, core_udf_test_env: Dict[str, Any]) -> None:
...
class TestUDFRegistration:
    ...
    def test_registration_scalar_udfs_can_be_called(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        result = engine.execute_query("SELECT format_name('John', 'Doe') as full_name").fetchdf()
    ...
    def test_registration_table_udfs_can_be_called(self, core_udf_test_env: Dict[str, Any]) -> None:
        """User registers table UDFs and uses them in SQL queries."""
        engine = DuckDBEngine(":memory:")
        manager = PythonUDFManager(project_dir=core_udf_test_env["project_dir"])
        ...
        result = engine.execute_query("""
            SELECT * FROM enrich_data(SELECT * FROM test_data)
            ORDER BY id
        """).fetchdf()
    ...
    def test_registration_handles_duplicate_names_gracefully(self, core_udf_test_env: Dict[str, Any]) -> None:
...
class TestUDFExecution:
    ...
    def test_execution_scalar_udf_with_real_data(self, core_udf_test_env: Dict[str, Any]) -> None:
    ...
    def test_execution_table_udf_with_aggregation(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        # Use table UDF for customer summary
        result = engine.execute_query("""
            SELECT * FROM customer_summary(SELECT * FROM orders)
    ...
    def test_execution_udfs_in_complex_query(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        result = engine.execute_query("""
            WITH enriched AS (
                SELECT * FROM revenue_analysis(SELECT * FROM sales)
            )
            SELECT 
                period,
                revenue,
        ...
        assert q2_data["growth_rate"] == 20.0  # (120k - 100k) / 100k * 100
        assert q2_data["adjusted_growth_rate"] == 30.0  # 20 + 10
...
class TestUDFQueryProcessing:
    ...
    def test_query_processing_with_variable_substitution(self, core_udf_test_env: Dict[str, Any]) -> None:
    ...
    def test_query_processing_preserves_udf_functionality(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        query = """
            WITH step1 AS (
                SELECT * FROM VALUES (1, 100.0), (2, 200.0) AS t(id, value)
            ),
            step2 AS (
                SELECT * FROM enrich_data(SELECT * FROM step1)  
            )
            SELECT 
                id,
                value,
...
class TestUDFNamespaceIsolation:
    ...
    def test_namespace_isolation_prevents_conflicts(self, core_udf_test_env: Dict[str, Any]) -> None:
    ...
    def test_namespace_isolation_with_engine_registration(self, core_udf_test_env: Dict[str, Any]) -> None:
        ...
        result2 = engine.execute_query("SELECT calculate_growth_rate(120, 100) as growth").fetchdf()
